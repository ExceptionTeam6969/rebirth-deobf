...
public class MixinTransformer extends TreeTransformer
{
    ...
    private final List pendingConfigs;
    ...
    public synchronized byte[] transformClassBytes(final String s, final String s2, byte[] array) {
        ...
        try {
            if (this.postProcessor.canTransform(s2)) {
                ...
                final byte[] array2 = transformClassBytes;
                this.lock.pop();
                begin2.end();
                return array2;
            }
            ...
            for (final MixinConfig mixinConfig : this.configs) {
                if (mixinConfig.packageMatch(s2)) {
                    ...
                    set.addAll(mixinConfig.getMixinsFor(s2));
                }
            }
            if (b) {
    ...
    public List reload(final String s, final byte[] array) {
    ...
    private void selectConfigs(final MixinEnvironment mixinEnvironment) {
        ...
        Collections.sort((List<Comparable>)this.pendingConfigs);
    }
    ...
    private int prepareConfigs(final MixinEnvironment mixinEnvironment) {
        ...
        for (final MixinConfig mixinConfig : this.pendingConfigs) {
            ...
            if (hotSwapper != null) {
                mixinConfig.addListener((MixinConfig.IListener)new MixinConfig.IListener(hotSwapper) {
        ...
        for (final MixinConfig mixinConfig3 : this.pendingConfigs) {
            ...
            for (final MixinConfig mixinConfig4 : this.pendingConfigs) {
                if (!mixinConfig4.equals(mixinConfig3)) {
                    set.addAll(mixinConfig4.getTargets());
                }
            }
            plugin.acceptTargets(mixinConfig3.getTargets(), (Set)Collections.unmodifiableSet((Set<?>)set));
        }
        ...
        Collections.sort((List<Comparable>)this.configs);
    ...
    private void handleMixinError(final String s, final InvalidMixinException ex, final MixinEnvironment mixinEnvironment, final ErrorPhase errorPhase) throws Error {
        ...
        final Iterator iterator = this.getErrorHandlers(mixin.getPhase()).iterator();
        while (iterator.hasNext()) {
    ...
    private List getErrorHandlers(final MixinEnvironment.Phase phase) {
        ...
        for (final String s : Mixins.getErrorHandlerClasses()) {
            try {
                ...
                final IMixinErrorHandler mixinErrorHandler = this.service.getClassProvider().findClass(s, true).newInstance();
    ...
    enum ErrorPhase
    {
        ...
        private ErrorPhase(final String s, final int n) {
