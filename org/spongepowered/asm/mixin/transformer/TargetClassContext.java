...
class TargetClassContext extends ClassContext implements ITargetClassContext
{
    ...
    private final Map targetMethods;
    private final Set mixinMethods;
    private int nextUniqueMethodIndex;
    private int nextUniqueFieldIndex;
    private boolean applied;
    private boolean forceExport;
    
    TargetClassContext(final MixinEnvironment env, final Extensions extensions, final String sessionId, final String className, final ClassNode classNode, final SortedSet mixins) {
        this.targetMethods = new HashMap();
        this.mixinMethods = new HashSet();
        this.env = env;
    ...
    MethodNode findMethod(final Deque deque, final String s) {
        return this.findAliasedMethod(deque, s, true);
    }
    
    MethodNode findAliasedMethod(final Deque deque, final String s) {
        return this.findAliasedMethod(deque, s, false);
    }
    
    private MethodNode findAliasedMethod(final Deque deque, final String s, final boolean b) {
    ...
    FieldNode findAliasedField(final Deque deque, final String s) {
        final String s2 = deque.poll();
        if (s2 == null) {
            return null;
        }
