...
public class InjectorGroupInfo
{
    ...
    public Collection getMembers() {
        return Collections.unmodifiableCollection((Collection<?>)this.members);
    }
    ...
    public static final class Map extends HashMap
    {
        ...
        public InjectorGroupInfo parseGroup(final AnnotationNode annotationNode, final String s) {
            ...
            String s2 = (String)Annotations.getValue(annotationNode, "name");
            if (s2 == null || s2.isEmpty()) {
                s2 = s;
            }
            final InjectorGroupInfo forName = this.forName(s2);
            final Integer n = (Integer)Annotations.getValue(annotationNode, "min");
            if (n != null && n != -1) {
                forName.setMinRequired(n);
            }
            final Integer n2 = (Integer)Annotations.getValue(annotationNode, "max");
            if (n2 != null && n2 != -1) {
        ...
        public void validateAll() throws InjectionValidationException {
            final Iterator<InjectorGroupInfo> iterator = this.values().iterator();
            while (iterator.hasNext()) {
