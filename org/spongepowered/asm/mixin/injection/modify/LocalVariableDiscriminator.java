...
public class LocalVariableDiscriminator
{
    ...
    public LocalVariableDiscriminator(final boolean argsOnly, final int ordinal, final int index, final Set set, final boolean print) {
        this.argsOnly = argsOnly;
        this.ordinal = ordinal;
        this.index = index;
        this.names = Collections.unmodifiableSet((Set<?>)set);
        this.print = print;
    }
    ...
    public static LocalVariableDiscriminator parse(final AnnotationNode annotationNode) {
        final boolean booleanValue = (boolean)Annotations.getValue(annotationNode, "argsOnly", Boolean.FALSE);
        final int intValue = (int)Annotations.getValue(annotationNode, "ordinal", -1);
        final int intValue2 = (int)Annotations.getValue(annotationNode, "index", -1);
        final boolean booleanValue2 = (boolean)Annotations.getValue(annotationNode, "print", Boolean.FALSE);
        final HashSet set = new HashSet();
        final List list = (List)Annotations.getValue(annotationNode, "name", (Object)null);
        if (list != null) {
            set.addAll(list);
        }
        return new LocalVariableDiscriminator(booleanValue, intValue, intValue2, set, booleanValue2);
    }
        ...
        public void print(final PrettyPrinter prettyPrinter) {
            ...
            for (int i = this.baseArgIndex; i < this.locals.length; ++i) {
                ...
                if (local != null) {
                    ...
                    prettyPrinter.add("[%3d]    [%3d]  %30s  %-50s  %s", i, local.ord, SignaturePrinter.getTypeName(type, false), local.name, this.returnType.equals((Object)type) ? "YES" : "-");
                }
                else if (i > 0) {
