...
public class CallbackInjector extends Injector
{
    ...
    private MethodNode generateErrorMethod(final Callback callback, final String s, final String s2) {
        ...
        instructions.add((AbstractInsnNode)new TypeInsnNode(187, s));
        instructions.add((AbstractInsnNode)new InsnNode(89));
        instructions.add((AbstractInsnNode)new LdcInsnNode((Object)s2));
        instructions.add((AbstractInsnNode)new MethodInsnNode(183, s, "<init>", "(Ljava/lang/String;)V", false));
    ...
    protected void instanceCallbackInfo(final Callback callback, final String lastId, final String lastDesc, final boolean b) {
        ...
        callback.add((AbstractInsnNode)new TypeInsnNode(187, this.callbackInfoClass), true, !b, b2);
        callback.add((AbstractInsnNode)new InsnNode(89), true, true, b2);
        callback.add((AbstractInsnNode)new LdcInsnNode((Object)lastId), true, !b, b2);
        callback.add((AbstractInsnNode)new InsnNode(this.cancellable ? 4 : 3), true, !b, b2);
        if (callback.isAtReturn) {
            callback.add((AbstractInsnNode)new VarInsnNode(callback.target.returnType.getOpcode(21), callback.marshalVar()), true, !b);
            callback.add((AbstractInsnNode)new MethodInsnNode(183, this.callbackInfoClass, "<init>", lastDesc, false));
        }
        else {
            callback.add((AbstractInsnNode)new MethodInsnNode(183, this.callbackInfoClass, "<init>", lastDesc, false), false, false, b2);
        }
        if (b) {
            callback.target.addLocalVariable(this.callbackInfoVar, "callbackInfo" + this.callbackInfoVar, "L" + this.callbackInfoClass + ";");
            callback.add((AbstractInsnNode)new VarInsnNode(58, this.callbackInfoVar), false, false, b2);
        }
    }
    ...
    private String getIdentifier(final Callback callback) {
        ...
        final String s2 = this.ids.get(callback.node.getId());
        return s + (Strings.isNullOrEmpty(s2) ? "" : (":" + s2));
    }
    ...
    protected void injectCancellationCode(final Callback callback) {
        ...
        callback.add((AbstractInsnNode)new JumpInsnNode(153, labelNode));
        this.injectReturnCode(callback);
        callback.add((AbstractInsnNode)labelNode);
    }
    ...
    protected void injectReturnCode(final Callback callback) {
        if (callback.target.returnType.equals((Object)Type.VOID_TYPE)) {
            ...
            callback.add((AbstractInsnNode)new MethodInsnNode(182, this.callbackInfoClass, CallbackInfoReturnable.getReturnAccessor(callback.target.returnType), CallbackInfoReturnable.getReturnDescriptor(callback.target.returnType), false));
    ...
    private static List summariseLocals(final String s, final int n) {
        return summariseLocals(Type.getArgumentTypes(s), n);
    }
    
    private static List summariseLocals(final Type[] array, int i) {
    ...
    private class Callback extends InsnList
    {
        ...
        Callback(final CallbackInjector this$0, final MethodNode handler, final Target target, final InjectionNodes.InjectionNode node, final LocalVariableNode[] locals, final boolean b) {
            ...
            this.isAtReturn = (this.node.getCurrentTarget() instanceof InsnNode && this >= this.node.getCurrentTarget().getOpcode());
        ...
        boolean checkDescriptor(final String s) {
            ...
            for (int i = 0; i < argumentTypes2.length; ++i) {
                ...
                if (!type.equals((Object)argumentTypes2[i])) {
                    if (type.getSort() == 9) {
                        return false;
                    }
