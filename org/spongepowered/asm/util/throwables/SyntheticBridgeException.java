...
public class SyntheticBridgeException extends MixinException
{
    ...
    public void printAnalysis(final IMixinContext mixinContext, final MethodNode methodNode, final MethodNode methodNode2) {
        ...
        prettyPrinter.add().kv("Method", (Object)(this.name + this.desc)).kv("Problem Type", (Object)this.problem).add().hr();
        final String s = (String)Annotations.getValue(Annotations.getVisible(methodNode, (Class)MixinMerged.class), "mixin");
    ...
    private PrettyPrinter printProblem(final PrettyPrinter prettyPrinter, final IMixinContext mixinContext, final MethodNode methodNode, final MethodNode methodNode2) {
        ...
        switch (this.problem) {
            ...
            case BAD_LOAD: {
                ...
                final ListIterator iterator = methodNode.instructions.iterator();
                final ListIterator iterator2 = methodNode2.instructions.iterator();
                ...
                while (iterator.hasNext() && iterator2.hasNext()) {
                    ...
                    if (abstractInsnNode instanceof VarInsnNode && abstractInsnNode2 instanceof VarInsnNode) {
                        ...
                        prettyPrinter.kv("Target " + n, "%8s %-2d %s", new Object[] { Bytecode.getOpcodeName((AbstractInsnNode)varInsnNode), varInsnNode.var, type });
                        prettyPrinter.kv("Incoming " + n, "%8s %-2d %s", new Object[] { Bytecode.getOpcodeName((AbstractInsnNode)varInsnNode2), varInsnNode2.var, type2 });
                        if (type.equals((Object)type2)) {
                            prettyPrinter.kv("", "Types match: %s", new Object[] { type });
                        }
                        else if (type.getSort() != type2.getSort()) {
                            prettyPrinter.kv("", (Object)"Types are incompatible");
                        }
                        else if (type.getSort() == 10) {
                            prettyPrinter.kv("", "Common supertype: %s", new Object[] { ClassInfo.getCommonSuperClassOrInterface(type, type2) });
                        }
                        prettyPrinter.add();
                    }
                    ++n;
            ...
            case BAD_CAST: {
                ...
                prettyPrinter.kv("Target type", (Object)objectType2);
                prettyPrinter.kv("Incoming type", (Object)objectType3);
                prettyPrinter.kv("Common supertype", (Object)ClassInfo.getCommonSuperClassOrInterface(objectType2, objectType3)).add();
                break;
            }
            ...
            case BAD_INVOKE_DESC: {
                ...
                if (argumentTypes3.length != argumentTypes4.length) {
                    ...
                    prettyPrinter.add("the target method has %d arguments, the incoming method has %d. This is an", new Object[] { argumentTypes3.length, argumentTypes4.length });
                    prettyPrinter.add("unrecoverable error. %s method has the expected arg count of %d", new Object[] { s, length });
                    break;
                }
    ...
    private PrettyPrinter printTypeComparison(final PrettyPrinter prettyPrinter, final String s, final Type type, final Type type2) {
        prettyPrinter.kv("Target " + s, "%s", new Object[] { type });
        prettyPrinter.kv("Incoming " + s, "%s", new Object[] { type2 });
        if (type.equals((Object)type2)) {
            prettyPrinter.kv("Analysis", "Types match: %s", new Object[] { type });
        }
        else if (type.getSort() != type2.getSort()) {
            prettyPrinter.kv("Analysis", (Object)"Types are incompatible");
        }
        else if (type.getSort() == 10) {
            prettyPrinter.kv("Analysis", "Common supertype: L%s;", new Object[] { ClassInfo.getCommonSuperClassOrInterface(type, type2) });
        }
        return prettyPrinter.add();
    }
    ...
    public enum Problem
    {
        BAD_INSN("BAD_INSN", 0, "Conflicting opcodes %4$s and %5$s at offset %3$d in synthetic bridge method %1$s%2$s"), 
        BAD_LOAD("BAD_LOAD", 1, "Conflicting variable access at offset %3$d in synthetic bridge method %1$s%2$s"), 
        BAD_CAST("BAD_CAST", 2, "Conflicting type cast at offset %3$d in synthetic bridge method %1$s%2$s"), 
        BAD_INVOKE_NAME("BAD_INVOKE_NAME", 3, "Conflicting synthetic bridge target method name in synthetic bridge method %1$s%2$s Existing:%6$s Incoming:%7$s"), 
        BAD_INVOKE_DESC("BAD_INVOKE_DESC", 4, "Conflicting synthetic bridge target method descriptor in synthetic bridge method %1$s%2$s Existing:%8$s Incoming:%9$s"), 
        BAD_LENGTH("BAD_LENGTH", 5, "Mismatched bridge method length for synthetic bridge method %1$s%2$s unexpected extra opcode at offset %3$d");
        
        private final String message;
        private static final Problem[] $VALUES;
        
        private Problem(final String s, final int n, final String message) {
