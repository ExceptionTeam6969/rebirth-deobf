...
public class ClassSignature
{
    ...
    private final Deque rawInterfaces;
    
    ClassSignature() {
        this.types = new LinkedHashMap();
        this.superClass = new Token("java/lang/Object");
        this.interfaces = new ArrayList();
        this.rawInterfaces = new LinkedList();
    }
    ...
    protected String getTypeVar(final TokenHandle tokenHandle) {
        for (final Map.Entry<TypeVar, V> entry : this.types.entrySet()) {
            final TypeVar typeVar = entry.getKey();
            final TokenHandle tokenHandle2 = (TokenHandle)entry.getValue();
    ...
    protected void addRawInterface(final String s) {
        ...
        final String type = token.asType(true);
        final Iterator<Token> iterator = (Iterator<Token>)this.interfaces.iterator();
    ...
    public void merge(final ClassSignature classSignature) {
        ...
        for (final Map.Entry<TypeVar, V> entry : classSignature.types.entrySet()) {
            this.addTypeVar(entry.getKey(), (TokenHandle)entry.getValue());
        }
    ...
    private void conform(final Set set) {
        for (final TypeVar typeVar : this.types.keySet()) {
    ...
    private String findUniqueName(final String s, final Set set) {
        if (!set.contains(s)) {
            return s;
        }
        if (s.length() == 1) {
    ...
    private String findOffsetName(final char c, final Set set) {
        return this.findOffsetName(c, set, "", "");
    }
    
    private String findOffsetName(final char c, final Set set, final String s, final String s2) {
    ...
    public String toString() {
        ...
        if (this.types.size() > 0) {
            ...
            for (final Map.Entry<K, TokenHandle> entry : this.types.entrySet()) {
