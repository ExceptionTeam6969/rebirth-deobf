...
public class AnnotatedMixinElementHandlerSoftImplements extends AnnotatedMixinElementHandler
{
    ...
    public void process(final AnnotationHandle annotationHandle) {
        ...
        final List annotationList = annotationHandle.getAnnotationList("value");
        ...
        for (final AnnotationHandle annotationHandle2 : annotationList) {
            final Interface.Remap remap = (Interface.Remap)annotationHandle2.getValue("remap", Interface.Remap.ALL);
            if (remap == Interface.Remap.NONE) {
                continue;
            }
            try {
                this.processSoftImplements(remap, new TypeHandle((DeclaredType)annotationHandle2.getValue("iface")), (String)annotationHandle2.getValue("prefix"));
            }
            catch (Exception ex) {
    ...
    private void addInterfaceMethodMapping(final Interface.Remap remap, final TypeHandle typeHandle, final String s, final MethodHandle methodHandle, final String s2, final String s3) {
        final ObfuscationData obfMethod = this.obf.getDataProvider().getObfMethod(new MappingMethod(typeHandle.getName(), s2, s3));
    ...
    private ObfuscationData applyPrefix(final ObfuscationData obfuscationData, final String s) {
        if (s == null) {
            return obfuscationData;
        }
        final ObfuscationData obfuscationData2 = new ObfuscationData();
        for (final ObfuscationType obfuscationType : obfuscationData) {
            obfuscationData2.put(obfuscationType, ((MappingMethod)obfuscationData.get(obfuscationType)).addPrefix(s));
        }
